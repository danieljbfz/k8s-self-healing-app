...
# The first thing we need to do is create a Flask application instance.
# This instance will serve as the main entry point for our application and is responsible
# for handling incoming requests and routing them to the appropriate view functions.
# In Flask, view functions are defined using the @app.route() decorator, which registers them 
# to a specific URL path. These functions behave very much like a controller in a MVC architecture, 
# handling the business logic and returning a response to the client.
app = Flask(__name__)

# In cloud-native environments like Kubernetes, configuration (ConfigMaps) and 
# sensitive data (Secrets) are decoupled from the application image. These values 
# are typically injected as environment variables at deployment time, allowing 
# settings to be changed without needing to rebuild or redeploy the application code. 
# The application retrieves these values using standard OS functions (e.g., `os.getenv()`)
APP_VERSION = os.getenv('APP_VERSION', '1.0.0')
APP_ENV = os.getenv('APP_ENV', 'development')

# We will now record the exact time the application process started, so we can later
# calculate and report its uptime for monitoring and debugging purposes.
...
def get_pod_info() -> dict:
    """
    Retrieves identifying network information for the current container instance (Pod).

    When this application runs inside a Kubernetes cluster, this function identifies 
    the specific **Pod** (the smallest deployable unit) handling the request. This 
    information is extremely useful for debugging and tracing requests when running 
    multiple replicas behind a load balancer.
    
    The OS hostname typically corresponds to the **Pod Name**, and the IP 
    is the **Pod IP** (which is internal to the cluster network).

    :return: A dictionary containing the hostname (Pod Name) and IP address of the container.
    :rtype: dict
    """
...
def get_uptime() -> str:
    ...
    This metric is used by monitoring systems to identify when a container 
    was last restarted by Kubernetes, whether intentionally (e.g., deployment)
    or unintentionally (e.g., a crash detected by the liveness probe).
    Every time the container restarts, this uptime counter resets to zero, 
    so a consistently low uptime can be a sign of frequent crashes 
    or service disruptions (a "crash loop").

    :return: A formatted string representing the uptime (e.g., "1 day, 2:03:45").
    :rtype: str
    """
...
def health():
    ...
    **programmatic access** by monitoring tools and service orchestrators, as opposed to 
    ...
    """
    # In this example, we provide a simple "always OK" health check.
    # In a real system, you would add logic here to check external dependencies,
    # like pinging the database or checking queue depth. If any check fails,
    # you would return a 503 Service Unavailable.
    # Do not add any heavy logic that might slow down the response, as this check
    # is typically performed many times per minute.
    # try: 
    #     db_connection.ping()
    # except: 
...
def crash():
    ...
    **NEVER** be included in production code.
    ...
    2. Immediately afterward, the process calls `os._exit(1)`.  
       This halts the interpreter *instantly*, bypassing all cleanup,
       buffer flushing, and finally/teardown handlers.
    3. Because the process terminates before the response is returned,
       the WSGI server never has a chance to write any bytes to the
       network socket. Therefore, the client **will not receive**
       the JSON message or even the HTTP headers.
    4. The orchestrator (e.g., Kubernetes) detects the non-zero exit code (1) 
       and, depending on its restart policy, replaces the failed container 
...
def metrics():
    ...
    * **Autoscaling:** Providing data (e.g., CPU usage) for Horizontal
      Pod Autoscalers (HPA) to dynamically manage resource allocation.
...
def internal_error(error):
    ...
    logic. Instead of exposing raw stack traces, we return a generic, 
    safe JSON response to the client with the 500 status code, logging the 
    details internally for debugging.
    
    NOTE: Detailed error information should be logged, not returned to the client.

    :param error: The underlying 500 error object.
    :return: A JSON response with a 500 status code.
    """
    # In a real app, you would log the full traceback here before returning the generic error.
    # logging.exception() is used inside an exception handler to log the 
    # full traceback of the most recently caught exception (the 500 error).
...
if __name__ == '__main__':
    ...
    # Configuration Check
    # ------------------------------------------------------------------------
    # We retrieve the port from an environment variable (standard for containerization)
    # or default to the common Flask port 5000.
